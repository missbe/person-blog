...
<script>
  ...
  $(document).ready(function() {
        oTable = $('#${className}Table').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<%=basePath%>user/roles/${className}/list",
            order:[[0, 'desc']],
            dom: 'Bfrtip',
            select: {
                style: 'multi'
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        window.location.href="<%=basePath%>user/roles/${className}/addUI";
                    }
                },
                {
                    ...
                    action: function () {
                        ...
                        if(selected.length!=1){
                            ...
                            window.location.href="<%=basePath%>user/roles/${className}/addUI?id="+selected[0][0];
                        }
                    }
                },
                {
                    ...
                    action: function (dom) {
                        ...
                        if(selected.length==0){
                            ...
                            bootstrap_confirm("确认删除以下"+selected.length+"项吗？",text,function(){
                                ...
                                submit(data, "user/roles/${className}/delete", dom, function(res){
                                    if(res.success){
