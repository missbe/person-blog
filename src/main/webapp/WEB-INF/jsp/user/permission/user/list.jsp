...
<script>
  ...
  $(document).ready(function() {
        oTable = $('#${className}Table').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<%=basePath%>user/users/${className}/list",
            order:[[0, 'desc']],
            dom: 'Bfrtip',
            select: {
                style: 'multi'
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        window.location.href="<%=basePath%>user/users/${className}/addUI";
                    }
                },
                {
                    ...
                    action: function () {
                        ...
                        if(selected.length!=1){
                            ...
                            window.location.href="<%=basePath%>user/users/${className}/addUI?id="+selected[0][0];
                        }
                    }
                },
                {
                    ...
                    action: function (dom) {
                        ...
                        if(selected.length==0){
                            ...
                            bootstrap_confirm("确认删除以下"+selected.length+"项吗？",text,function(){
                                ...
                                submit(data, "user/users/${className}/delete", dom, function(res){
                                    if(res.success){
                ...
                },
                {
                    ...
                    action: function (dom) {
                        ...
                        if(selected.length==0){
                            ...
                            bootstrap_confirm("确认锁定/取消锁定以下用户"+selected.length+"项吗？",text,function(){
                                var ids = new Array(selected.length);
                                for(var i =0 ; i<selected.length;i++){
                                    ids[i] = selected[i][0];
                                }
                                ...
                                submit(data, "user/users/${className}/changeStatus", dom, function(res){
                                    if(res.success){
                                        oTable.ajax.reload( null, false );
                                    }
                                });
