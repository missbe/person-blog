...
<script>
  ...
  $(document).ready(function() {
        oTable = $('#${className}Table').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<%=basePath%>user/themes/${className}/list",
            order:[[0, 'desc']],
            dom: 'Bfrtip',
            select: {
                style: 'multi'
                    ...
                    action: function (dom) {
                        ...
                        if(selected.length==0){
                            ...
                            bootstrap_confirm("确认审核通过/审核不通过以下"+selected.length+"项吗？",text,function(){
                                ...
                                submit(data, "user/themes/${className}/changeRecommend", dom, function(res){
                                    if(res.success){
                    ...
                    action: function (dom) {
                        ...
                        if(selected.length==0){
                            ...
                            bootstrap_confirm("确认删除以下"+selected.length+"项吗？",text,function(){
                                ...
                                submit(data, "user/themes/${className}/delete", dom, function(res){
                                    if(res.success){
                                        oTable.ajax.reload( null, false );
                                    }
                                });
